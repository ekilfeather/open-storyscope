...
* @link http://js-xpath.sourceforge.net/
* @modified Pavel Pěnkava
*/
...
if (isIE && IEversion >= 5)
{
  ...
  // HTMLElement getElementByReference(IXMLDOMNode reference)
  document.getElementByReference = function
    (
    reference      // IXMLDOMNode
    )
  {
    // return HTMLElement
    for (var i = 0; i < document.lookupTable.length; i++)
      if (document.lookupTable[i].XMLnode == reference)
        return document.lookupTable[i].HTMLnode;
    return null;
  }
      ...
      )
  {
    ...
    function findNode(result, current)
    {
      ...
      var node = document.getElementByReference(current);
      if (node)
      { // Return found node
        return node;
      }
      ...
      { // Node not found
        XPathException.Throw("unable to locate node in XML tree");
      }
    }
    ...
    function activateResult(result)
    {
      if (!result._domResult)
      {
        try
        {
          ...
          {
            result._domResult = document._XPathMsxmlDocumentHelper.getDom(result._contextNode).selectNodes(expression);
          }
          else
          {
            result._domResult = true;
            result._textResult = document._XPathMsxmlDocumentHelper.getTextResult(expression, result._contextNode);
          }
  ...
  {
    this.getDom = function(context)
    {
      if (typeof context == "undefined")
        context = document;
      activateDom(this, context);
      this.dom.setProperty("SelectionLanguage", "XPath");
      return this.dom;
    }
    
    this.getTextResult = function(expression, context)
    {
      ...
      try
      {
        var result = this.getDom(context).transformNode(xsl);
      }
      catch(error)
      {
    ...
    function activateDom(helper, context)
    {
      if (!helper.dom)
      {
        ...
        document.lookupTable = new Array();
        loadDocument(dom, helper, context);
        helper.dom = dom;
    ...
    function loadDocument(dom, helper, context)
    {
      return loadNode(dom, dom, context.documentElement, helper);
    }
    ...
    function loadNode(dom, domParentNode, node, helper)
    {
      if (node.nodeType == 3)
      {  // TEXT Node
        var domNode = dom.createTextNode(node.nodeValue);
        document.lookupTable.push({'HTMLnode': node, 'XMLnode': domNode}); // Reference to XML node
        domParentNode.appendChild(domNode);
      }
      else if (node.nodeType == 8)
      { // COMMENT Node
        var domNode = dom.createComment(node.nodeValue);
        document.lookupTable.push({'HTMLnode': node, 'XMLnode': domNode}); // Reference to XML node
        domParentNode.appendChild(domNode);
      }
      else
      { // ELEMENT Node
        ...
        if (node.nodeName == "IFRAME" && node.contentDocument && node.contentDocument.documentElement)
        { // Load nodes from document inside IFRAME
          loadNode(dom, domNode, node.contentDocument.documentElement, helper);
        }
        else
        { // Load child nodes
